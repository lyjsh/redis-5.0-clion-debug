# redis-5.0-clion-debug
redis 5.0基于clion可本地调试的版本
开发环境：
win10，开启wsl2.0虚拟子系统并安装应用商店linux子系统ubuntu24版本，安装gcc、make等基本编译环境。
安装clion2025 windows版本
在ubuntu子系统拉取源代码进入redis项目根目录执行make看是否能编译成功（参考redis官方使用）。
编译成功后打开clion使用wsl方式创建一个普通项目，创建成功表示环境已经配置完成，在当前项目界面使用拉取git方式新建本项目，完成后界面会提示使用cmake或者make方式导入，选择cmake进入项目界面。如果未识别在最外层的cmakelist.txt右键选择load cmake project或者reload cmake project。更新完成后会出现cmake-build-debug目录，在右边debug按钮选择redis-server进行启动。
